# Workflow diario: Hyprland + Polar + Kitty
...
- Apps:
  ...
  - `SUPER + B`: browser
  - `SUPER + D`: launcher
  - `SUPER + C`: Control Center Polar (AGS solo durante recuperacion)
- Ventanas/sesion:
...
5. Validar estructura y arranque sin cambiar runtime:
   - `bash bootstrap/quickshell-check.sh`
   - `bash bootstrap/test-desktop-shell.sh`
6. Probar multimedia:
...
- `git restore config/hypr/conf.d/50-binds.conf config/kitty/kitty.conf config/hypr/conf.d/90-autostart.conf config/hypr/scripts/bootstrap-workspaces.sh README.md docs/operations/daily-workflow.md`
